...
class MigrateHidemaru():
	...
	def main(self,path_input):
		...
		try:
			...
			# 正解データとソースデータを取得する
			data=self.load_nodes(path_text=path_input)
	...
	def each(self,text_source,path_input):
		'''
		メインの処理を実行します。
		'''
...
例1：全件処理をする場合）python2 migrate_hidemaru.py --path_input "data/hidemaru/nodes.txt"
例2：1つのキーワードだけで処理する場合）python2 migrate_hidemaru.py --path_input "data/hidemaru/nodes.txt" -k "CMV抗体/陰性"
		'''
	)
	parser.add_argument('-i','--path_input' ,help='テスト用データのパス',default=None)
	parser.add_argument('-k','--keyword' ,help='個別確認したいキーワード',default=None)
	kwargs=vars(parser.parse_args())

	if kwargs['keyword']:
		text=MigrateHidemaru(**kwargs).each(
			path_input=kwargs['path_input'],
			text_source=kwargs['keyword'].decode('utf-8')
		)
