...
def expand_dot(ds, dot_pat):
    ...
    # A1・A2・A3
    if re.match(dot_pat[0], text):
        text = text.replace(u"・", u"＆")
    # A1=B1/C1@D1・A2=B2/C2@D2・A3=B3/C3@D3
    elif re.match(dot_pat[1], text):
        text = text.replace(u"・", u"＆")
    # A1=B1@D1・A2=B2@D2・A3=B3@D3
    elif re.match(dot_pat[2], text):
        text = text.replace(u"・", u"＆")
    # A1=B1/C1・A2=B2/C2・A3=B3/C3
    elif re.match(dot_pat[3], text):
        text = text.replace(u"・", u"＆")
    # B1@D1・B2@D2・B3@D3
    elif re.match(dot_pat[4], text):
        text = text.replace(u"・", u"＆")
    # B1/C1・B2/C2・B3/C3
    elif re.match(dot_pat[5], text):
        text = text.replace(u"・", u"＆")
    # A=B1@C1・B2@C2
    elif re.match(dot_pat[6], text):
        text = product_text(text, eq=True)
    # A=B1/C1・B2/C2
    elif re.match(dot_pat[7], text):
        text = product_text(text, eq=True)
    # A1・A2・A3=B/C@D
    elif re.match(dot_pat[8], text):
        text = product_text(text, eq=True)
    # A1・A2・A3=B@D
    elif re.match(dot_pat[9], text):
        text = product_text(text, at=True, sl=True, eq=True)
    # A1・A2・A3=B/C
    elif re.match(dot_pat[10], text):
        text = product_text(text, at=True, eq=True)
    # A1・A2・A3=B
    elif re.match(dot_pat[11], text):
        text = product_text(text, eq=True)
    # B1・B2・B3@D
    elif re.match(dot_pat[12], text):
        text = product_text(text, at=True)
    # B1・B2・B3/C
    elif re.match(dot_pat[13], text):
        text = product_text(text, sl=True)
    # A=B1・B2・B3/C@D
    elif re.match(dot_pat[14], text):
        text = product_text(text, at=True, sl=True, eq=True)
    # A=B1・B2・B3@D
    elif re.match(dot_pat[15], text):
        text = product_text(text, at=True, eq=True)
    # A=B1・B2・B3/C
    elif re.match(dot_pat[16], text):
        text = product_text(text, sl=True, eq=True)
    # A=D1・D2・D3
    elif re.match(dot_pat[17], text):
        text = product_text(text, eq=True)
    # A=B/C@D1・D2・D3
    elif re.match(dot_pat[18], text):
        text = product_text(text, at=True)
    # A=B@D1・D2・D3
    elif re.match(dot_pat[19], text):
        text = product_text(text, at=True)
    # B@D1・D2・D3
    elif re.match(dot_pat[20], text):
        text = product_text(text, at=True)
    # B1・B2/C@D
    elif re.match(dot_pat[21], text):
        text = product_text(text, at=True, sl=True)
    # B/C@D1・D2・D3
    elif re.match(dot_pat[22], text):
        text = product_text(text, at=True)
    # H:A1・A2・A3
    elif re.match(dot_pat[23], text):
        text = product_text(text, cl=True)
    # H:A1・A2・A3@C
    elif re.match(dot_pat[24], text):
        text = product_text(text, at=True, cl=True)
    # H:A1・A2・A3/C
    elif re.match(dot_pat[25], text):
        text = product_text(text, sl=True, cl=True)
    # H:A@C1・C2
    elif re.match(dot_pat[26], text):
        text = product_text(text, at=True, cl=True)
    # A1・A2・A3=B/C@D1・D2・D3
    elif re.match(dot_pat[27], text):
        text = product_text(text, at=True, eq=True)
    # A1・A2・A3=B1・B2・B3/C@D
    elif re.match(dot_pat[28], text):
        text = product_text(text, at=True, sl=True, eq=True)
    # A=B1・B2・B3/C@D1・D2・D3
    elif re.match(dot_pat[29], text):
        text = product_text(text, at=True, eq=True)
    # A1・A2=B1・B2@C
    elif re.match(dot_pat[30], text):
        text = product_text(text, at=True, eq=True)
    # A1・A2=B1・B2
    elif re.match(dot_pat[31], text):
        text = product_text(text, eq=True)
    # A1・A2=B1@C1・C2
    elif re.match(dot_pat[32], text):
        text = product_text(text, at=True, eq=True)
    # A=B1・B2@C1・C2
    elif re.match(dot_pat[33], text):
        text = product_text(text, at=True, eq=True)
    # A1・A2=B1・B2/C
    elif re.match(dot_pat[34], text):
        text = product_text(text, at=True, sl=True, eq=True)
    # B1・B2/C@D1・D2
    elif re.match(dot_pat[35], text):
        text = product_text(text, at=True, sl=True, eq=True)
    # A1・A2@B1・B2
    elif re.match(dot_pat[36], text):
        text = product_text(text, at=True)
...
def main_process(ds):
    ds = replace_side_effects(ds)
    # ds = exclude_brackets(ds)
    ds = exclude_stars(ds)
    # ds = expand_dot(ds)
    return ds
